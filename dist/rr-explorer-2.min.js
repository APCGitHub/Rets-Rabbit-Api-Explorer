...
!function(){"use strict";function r(r,e){function a(){function a(){e.search(c.data.request).then(function(r){c.data.error=null,c.data.results=r},function(r){c.data.error=r.error})}function t(){c.data.searchForm.filter.push({value:"",operator:"",join:"and"})}function o(r){var e=c.data.searchForm.filter.indexOf(r);c.data.searchForm.filter.splice(e,1)}function l(){c.data.searchForm.orderby.push({value:""})}function i(){var e="",a=0,t=[];""!==c.data.searchForm.select&&(e+="$select="+c.data.searchForm.select),c.data.searchForm.filter.length&&(e+=""!==c.data.searchForm.select?"&$filter=":"$filter="),t=c.data.searchForm.filter.map(function(r){return r.value});for(var a=0;a<c.data.searchForm.filter.length;a++)e+=t[a],""!==c.data.searchForm.filter[a].value&&a+1<c.data.searchForm.filter.length&&(e+=" "+c.data.searchForm.filter[a].join),a+1<c.data.searchForm.filter.length&&(e+=" ");c.data.searchForm.orderby.length&&(e+=""!==c.data.searchForm.select&&c.data.searchForm.filter.length?"&$orderby=":"$orderby="),e+=c.data.searchForm.orderby.map(function(r){return""!==r.value?r.value+" "+r.direction:""}).join(", "),""!=c.data.searchForm.top&&(e+="$top="+c.data.searchForm.top),""!=c.data.searchForm.skip&&(e+="$skip="+c.data.searchForm.skip),c.data.fullRequest=r.apiUrl+"property?"+e,c.data.request=e}var c=this;c.data={searchForm:{select:"",filter:[{value:"",operator:"",join:"and"}],orderby:[{value:"",direction:"asc"}],top:"",skip:""},fullRequest:r.apiUrl+"property?",request:"",results:[],error:null},c.search=a,c.addFilter=t,c.removeFilter=o,c.addOrderby=l,c.updateQuery=i}return{restrict:"EA",scope:{},templateUrl:"explorer.bootstrap.html",controller:a,controllerAs:"vm",bindToController:!0}}angular.module("rr.api.v2.explorer.directive.explorer",[]).directive("apiExplorer",r),r.$inject=["ApiConfig","PropertyFactory"]}();
