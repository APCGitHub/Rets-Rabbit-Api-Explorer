...
!function(){"use strict";function r(r,e){var a=["$scope","$document","PropertyFactory",function(e,a,t){function o(r){if(r){var e=angular.element(document.getElementById("query-results"));a.scrollToElement(e,80,300),c.data.total_results=-1,c.data.error=null,c.data.results=null,c.data.searching=!0;var o=new Date;t.search(c.data.request).then(function(r){var e=new Date;c.data.query_time=e.getTime()-o.getTime(),c.data.error=null,c.data.results=r,c.data.total_results=r.value.length,c.data.searching=!1},function(r){c.data.query_time=-1,c.data.searching=!1,c.data.error=r.error})}}function l(){c.data.searchForm.filter.push({value:"",join:"and"})}function s(r){var e=c.data.searchForm.filter.indexOf(r);c.data.searchForm.filter.splice(e,1),d()}function i(){c.data.searchForm.orderby.push({value:"",direction:"asc"})}function n(r){var e=c.data.searchForm.orderby.indexOf(r);c.data.searchForm.orderby.splice(e,1),d()}function d(){var e="",a=0,t=[0,0,0,0,0];if(""!==c.data.searchForm.select&&(t[0]=1,e+="$select="+c.data.searchForm.select),c.data.searchForm.filter.length)for(t[1]=1,e+=t[0]?"&$filter=":"$filter=",a=0;a<c.data.searchForm.filter.length;a++)e+=c.data.searchForm.filter[a].value,""!==c.data.searchForm.filter[a].value&&a+1<c.data.searchForm.filter.length&&(e+=" "+c.data.searchForm.filter[a].join),a+1<c.data.searchForm.filter.length&&(e+=" ");c.data.searchForm.orderby.length&&(t[2]=1,e+=t[0]+t[1]>0?"&$orderby=":"$orderby=",e+=c.data.searchForm.orderby.map(function(r){return""!==r.value?r.value+" "+r.direction:""}).join(", ")),""!=c.data.searchForm.top&&(t[3]=1,e+=t[0]+t[1]+t[2]>0?"&$top=":"$top=",e+=c.data.searchForm.top),""!=c.data.searchForm.skip&&(t[4]=1,e+=t[0]+t[1]+t[2]+t[3]>0?"&$skip=":"$skip=",e+=c.data.searchForm.skip),c.data.fullRequest=r.apiUrl+"property?"+e,c.data.request=e}var c=this;e.$watch(angular.bind(this,function(){return this.search}),function(r){r&&"undefined"!==r&&void 0!==r&&"function"!=typeof r&&(c.data.searchForm.select=r.query.select,c.data.searchForm.filter=r.query.filter,c.data.searchForm.orderby=r.query.orderby,c.data.searchForm.top=r.query.top,c.data.searchForm.skip=r.query.skip,c.data.error=null,c.data.results=null,c.data.searching=!1,c.data.total_results=-1,d())}),c.data={searchForm:{select:"",filter:[{value:"",join:"and"}],orderby:[{value:"",direction:"asc"}],top:"",skip:""},fullRequest:r.apiUrl+"property?",request:"",results:null,total_results:-1,error:null,searching:!1,query_time:-1},c.doSearch=o,c.addFilter=l,c.removeFilter=s,c.addOrderby=i,c.removeOrderby=n,c.updateQuery=d}];return{restrict:"EA",scope:!0,link:function(r,e,a,t){},templateUrl:"explorer.bootstrap.html",controller:a,controllerAs:"vm",bindToController:{search:"=",loader:"="}}}angular.module("rr.api.v2.explorer.directive.explorer",[]).directive("apiExplorer",r),r.$inject=["ApiConfig","PropertyFactory"]}();
