...
!function(){"use strict";function a(a,e){var r=["$scope","$interval","$document","PropertyFactory","uiGmapGoogleMapApi","uiGmapIsReady",function(e,r,t,n,o,l){function i(a){if(a){h.data.total_results=-1,h.data.error=null,h.data.results=null,h.data.searching=!0;var e=new Date;n.search(h.data.request).then(function(a){t.scrollToElement(f,70,300);var r=new Date;h.data.error=null,h.data.results=a,h.data.total_results=a.value.length,h.data.searching=!1,p(r.getTime()-e.getTime())},function(a){t.scrollToElement(f,70,300),h.data.query_time=-1,h.data.searching=!1,h.data.error=a?a.error:{}})}}function d(){h.data.searchForm.filter.push({value:"",join:"and"})}function s(a){var e=h.data.searchForm.filter.indexOf(a);h.data.searchForm.filter.splice(e,1),u()}function g(){h.data.searchForm.orderby.push({value:"",direction:"asc"})}function c(a){var e=h.data.searchForm.orderby.indexOf(a);h.data.searchForm.orderby.splice(e,1),u()}function u(){var e="",r=0,t=[0,0,0,0,0];if(""!==h.data.searchForm.select&&(t[0]=1,e+="$select="+h.data.searchForm.select),h.data.searchForm.filter.length)for(t[1]=1,e+=t[0]?"&$filter=":"$filter=",r=0;r<h.data.searchForm.filter.length;r++)e+=h.data.searchForm.filter[r].value,""!==h.data.searchForm.filter[r].value&&r+1<h.data.searchForm.filter.length&&(e+=" "+h.data.searchForm.filter[r].join),r+1<h.data.searchForm.filter.length&&(e+=" ");h.data.searchForm.orderby.length&&(t[2]=1,e+=t[0]+t[1]>0?"&$orderby=":"$orderby=",e+=h.data.searchForm.orderby.map(function(a){return""!==a.value?a.value+" "+a.direction:""}).join(", ")),""!=h.data.searchForm.top&&(t[3]=1,e+=t[0]+t[1]+t[2]>0?"&$top=":"$top=",e+=h.data.searchForm.top),""!=h.data.searchForm.skip&&(t[4]=1,e+=t[0]+t[1]+t[2]+t[3]>0?"&$skip=":"$skip=",e+=h.data.searchForm.skip),h.data.fullRequest=a.apiUrl+"property?"+e,h.data.request=e}function p(a){h.data.query_time=0;var e=5;a>750?h.data.query_time=a:m=r(function(e){h.data.query_time==a?r.cancel(m):h.data.query_time++},e)}var m,h=this,f=angular.element(document.getElementById("rr-query-results")),y={fillColor:"#ec6952",fillOpacity:.5,editable:!0,draggable:!0,zIndex:1,clickable:!1,strokeWeight:3};e.$watch(angular.bind(this,function(){return this.search}),function(a){a&&"undefined"!==a&&void 0!==a&&"function"!=typeof a&&(h.data.searchForm.select=a.query.select,h.data.searchForm.filter=a.query.filter,h.data.searchForm.orderby=a.query.orderby,h.data.searchForm.top=a.query.top,h.data.searchForm.skip=a.query.skip,h.data.error=null,h.data.results=null,h.data.searching=!1,h.data.total_results=-1,u())}),h.data={searchForm:{select:"",filter:[{value:"",join:"and"}],orderby:[{value:"",direction:"asc"}],top:"",skip:""},map:{center:{latitude:39.9612,longitude:-82.9988},zoom:11,drawingManagerOptions:{},drawingManagerControl:{},bounds:{},shape:null},fullRequest:a.apiUrl+"property?",request:"",results:null,total_results:-1,error:null,searching:!1,query_time:-1},o.then(function(a){h.data.map.drawingManagerOptions={drawingMode:google.maps.drawing.OverlayType.MARKER,drawingControl:!0,drawingControlOptions:{position:google.maps.ControlPosition.TOP_CENTER,drawingModes:[google.maps.drawing.OverlayType.CIRCLE,google.maps.drawing.OverlayType.POLYGON,google.maps.drawing.OverlayType.RECTANGLE]},polygonOptions:{},circleOptions:{},rectangleOptions:{}},angular.extend(h.data.map.drawingManagerOptions.polygonOptions,y),angular.extend(h.data.map.drawingManagerOptions.circleOptions,y),angular.extend(h.data.map.drawingManagerOptions.rectangleOptions,y)}),l.promise().then(function(a){google.maps.event.addListener(h.data.map.drawingManagerControl.getDrawingManager(),"circlecomplete",function(a){var e=a.getRadius(),r={lat:a.center.lat(),lng:a.center.lng()};console.log(JSON.stringify(r)+" "+e)}),google.maps.event.addListener(h.data.map.drawingManagerControl.getDrawingManager(),"radius_changed",function(a){console.log("radius change");var e=a.getRadius(),r={lat:a.center.lat(),lng:a.center.lng()};console.log(JSON.stringify(r)+" "+e)}),google.maps.event.addListener(h.data.map.drawingManagerControl.getDrawingManager(),"polygoncomplete",function(a){for(var e=[],r=0;r<a.getPath().getLength();r++){var t=a.getPath().getAt(r);e.push({lat:t.lat(),lng:t.lng()})}}),google.maps.event.addListener(h.data.map.drawingManagerControl.getDrawingManager(),"rectanglecomplete",function(a){var e=[],r=a.getBounds(),t=r.getNorthEast(),n=r.getSouthWest();e.push({lat:t.lat(),lng:n.lng()}),e.push({lat:t.lat(),lng:t.lng()}),e.push({lat:n.lat(),lng:t.lng()}),e.push({lat:n.lat(),lng:n.lng()})}),google.maps.event.addListener(h.data.map.drawingManagerControl.getDrawingManager(),"dragend",function(a){console.log("dragged"),console.log(a)})}),h.doSearch=i,h.addFilter=d,h.removeFilter=s,h.addOrderby=g,h.removeOrderby=c,h.updateQuery=u}];return{restrict:"EA",scope:!0,link:function(a,e,r,t){},templateUrl:"explorer.bootstrap.html",controller:r,controllerAs:"vm",bindToController:{search:"=",loader:"="}}}angular.module("rr.api.v2.explorer.directive.explorer",[]).directive("apiExplorer",a),a.$inject=["ApiConfig","PropertyFactory"]}();
