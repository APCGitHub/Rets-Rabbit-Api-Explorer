...
!function(){"use strict";function a(a,e){var t=["$scope","$interval","$document","PropertyFactory","uiGmapGoogleMapApi","uiGmapIsReady",function(e,t,r,n,o,l){function i(a){if(a){M.data.total_results=-1,M.data.error=null,M.data.results=null,M.data.searching=!0;var e=new Date;n.search(M.data.request).then(function(a){r.scrollToElement(C,70,300);var t=new Date;M.data.error=null,M.data.results=a,M.data.total_results=a.value.length,M.data.searching=!1,h(t.getTime()-e.getTime())},function(a){r.scrollToElement(C,70,300),M.data.query_time=-1,M.data.searching=!1,M.data.error=a?a.error:{}})}}function s(){M.data.searchForm.filter.push({value:"",join:"and"})}function d(a){var e=M.data.searchForm.filter.indexOf(a);M.data.searchForm.filter.splice(e,1),p()}function g(){M.data.searchForm.orderby.push({value:"",direction:"asc"})}function c(a){var e=M.data.searchForm.orderby.indexOf(a);M.data.searchForm.orderby.splice(e,1),p()}function p(){var e="",t=0,r=[0,0,0,0,0];if(""!==M.data.searchForm.select&&(r[0]=1,e+="$select="+M.data.searchForm.select),M.data.searchForm.filter.length||M.data.searchForm.geo.within.distance>0){for(r[1]=1,e+=r[0]?"&$filter=":"$filter=",t=0;t<M.data.searchForm.filter.length;t++)e+=M.data.searchForm.filter[t].value,""!==M.data.searchForm.filter[t].value&&t+1<M.data.searchForm.filter.length&&(e+=" "+M.data.searchForm.filter[t].join),t+1<M.data.searchForm.filter.length&&(e+=" ");M.data.searchForm.geo.within.distance>0&&(M.data.searchForm.filter.length&&(e+=" and "),e+="geo.distance(location, POINT("+M.data.searchForm.geo.within.center.lng+" "+M.data.searchForm.geo.within.center.lat+") le "+M.data.searchForm.geo.within.distance,console.log("did hte distance")),M.data.searchForm.geo.intersects.points.length>0&&(M.data.searchForm.filter.length&&(e+=" and "),e+="geo.intersects(location, POLYGON((",e+=M.data.searchForm.geo.intersects.points.map(function(a){return a.lng+" "+a.lat}).join(", "),e+="))",console.log("did the intersects"))}M.data.searchForm.orderby.length&&(r[2]=1,e+=r[0]+r[1]>0?"&$orderby=":"$orderby=",e+=M.data.searchForm.orderby.map(function(a){return""!==a.value?a.value+" "+a.direction:""}).join(", ")),""!=M.data.searchForm.top&&(r[3]=1,e+=r[0]+r[1]+r[2]>0?"&$top=":"$top=",e+=M.data.searchForm.top),""!=M.data.searchForm.skip&&(r[4]=1,e+=r[0]+r[1]+r[2]+r[3]>0?"&$skip=":"$skip=",e+=M.data.searchForm.skip),console.log(e),M.data.fullRequest=a.apiUrl+"property?"+e,M.data.request=e}function h(a){M.data.query_time=0;var e=5;a>750?M.data.query_time=a:b=t(function(e){M.data.query_time==a?t.cancel(b):M.data.query_time++},e)}function u(a){var e=a.getRadius(),t={lat:a.center.lat(),lng:a.center.lng()};M.data.searchForm.geo.within.center=t,M.data.searchForm.geo.within.distance=e,p()}function m(a){M.data.searchForm.geo.intersects.points=a,p()}function f(a){switch(a){case 1:M.data.map.shape.rectangle=null,M.data.map.shape.polygon=null;break;case 2:M.data.map.shape.circle=null,M.data.map.shape.polygon=null;break;case 3:M.data.map.shape.circle=null,M.data.map.shape.rectangle=null}}function y(a){f(1);var e=null===M.data.map.shape.circle;M.data.map.shape.circle=a,u(M.data.map.shape.circle),e&&(M.data.map.shape.circle.addListener("radius_changed",function(){console.log("radius change"),u(M.data.map.shape.circle)}),M.data.map.shape.circle.addListener("center_changed",function(){console.log("circle center change"),u(M.data.map.shape.circle)}))}function v(a){f(2);var e=null===M.data.map.shape.rectangle;M.data.map.shape.rectangle=a;var t=[],r=a.getBounds(),n=r.getNorthEast(),o=r.getSouthWest();t.push({lat:n.lat(),lng:o.lng()}),t.push({lat:n.lat(),lng:n.lng()}),t.push({lat:o.lat(),lng:n.lng()}),t.push({lat:o.lat(),lng:o.lng()}),m(t),e&&google.maps.event.addListener(M.data.map.shape.rectangle,"bounds_changed",function(){console.log("bounds changed");var a=[],e=M.data.map.shape.rectangle.getBounds(),t=e.getNorthEast(),r=e.getSouthWest();a.push({lat:t.lat(),lng:r.lng()}),a.push({lat:t.lat(),lng:t.lng()}),a.push({lat:r.lat(),lng:t.lng()}),a.push({lat:r.lat(),lng:r.lng()}),m(a)})}function F(a){f(3);var e=null===M.data.map.shape.polygon;M.data.map.shape.polygon=a;for(var t=[],r=0;r<a.getPath().getLength();r++){var n=a.getPath().getAt(r);t.push({lat:n.lat(),lng:n.lng()})}m(t),e&&(google.maps.event.addListener(M.data.map.shape.polygon.getPath(),"set_at",function(){for(var e=[],t=0;t<a.getPath().getLength();t++){var r=a.getPath().getAt(t);e.push({lat:r.lat(),lng:r.lng()})}m(e)}),google.maps.event.addListener(M.data.map.shape.polygon.getPath(),"insert_at",function(){for(var e=[],t=0;t<a.getPath().getLength();t++){var r=a.getPath().getAt(t);e.push({lat:r.lat(),lng:r.lng()})}m(e)}),google.maps.event.addListener(M.data.map.shape.polygon.getPath(),"remove_at",function(){for(var e=[],t=0;t<a.getPath().getLength();t++){var r=a.getPath().getAt(t);e.push({lat:r.lat(),lng:r.lng()})}m(e)}))}function w(a,e){var t=document.createElement("div");t.style.backgroundColor="#fff",t.style.border="2px solid #fff",t.style.borderRadius="3px",t.style.boxShadow="0 2px 6px rgba(0,0,0,.3)",t.style.cursor="pointer",t.style.marginBottom="22px",t.style.marginRight="10px",t.style.marginTop="10px",t.style.textAlign="center",t.title="Click to delete current selected shape",a.appendChild(t);var r=document.createElement("div");r.style.color="rgb(25,25,25)",r.style.fontFamily="Roboto,Arial,sans-serif",r.style.fontSize="16px",r.style.lineHeight="38px",r.style.paddingLeft="5px",r.style.paddingRight="5px",r.innerHTML="Delete Shape",t.appendChild(r),t.addEventListener("click",function(){M.data.map.shape.circle&&M.data.map.shape.circle.setMap(null),M.data.map.shape.rectangle&&M.data.map.shape.rectangle.setMap(null),M.data.map.shape.polygon&&M.data.map.shape.polygon.setMap(null),M.data.searchForm.geo.within.distance=-1,M.data.searchForm.geo.within.center={},M.data.searchForm.geo.intersects.points=[],M.data.map.drawingManagerControl.getDrawingManager()&&M.data.map.drawingManagerControl.getDrawingManager().setOptions({drawingControl:!0}),p()})}var b,M=this,C=angular.element(document.getElementById("rr-query-results")),O={fillColor:"#ec6952",fillOpacity:.5,editable:!0,draggable:!0,strokeWeight:3,geodesic:!0};e.$watch(angular.bind(this,function(){return this.search}),function(a){a&&"undefined"!==a&&void 0!==a&&"function"!=typeof a&&(M.data.searchForm.select=a.query.select,M.data.searchForm.filter=a.query.filter,M.data.searchForm.orderby=a.query.orderby,M.data.searchForm.top=a.query.top,M.data.searchForm.skip=a.query.skip,M.data.error=null,M.data.results=null,M.data.searching=!1,M.data.total_results=-1,p())}),M.data={searchForm:{select:"",filter:[{value:"",join:"and"}],orderby:[{value:"",direction:"asc"}],top:"",skip:"",geo:{intersects:{points:[]},within:{center:{},distance:-1}}},map:{center:{latitude:39.9612,longitude:-82.9988},zoom:11,drawingManagerOptions:{},drawingManagerControl:{},bounds:{},shape:{circle:null,rectangle:null,polygon:null}},fullRequest:a.apiUrl+"property?",request:"",results:null,total_results:-1,error:null,searching:!1,query_time:-1},o.then(function(a){M.data.map.drawingManagerOptions={drawingMode:google.maps.drawing.OverlayType.MARKER,drawingControl:!0,drawingControlOptions:{position:google.maps.ControlPosition.TOP_CENTER,drawingModes:[google.maps.drawing.OverlayType.CIRCLE,google.maps.drawing.OverlayType.POLYGON,google.maps.drawing.OverlayType.RECTANGLE]},polygonOptions:O,circleOptions:O,rectangleOptions:O}}),l.promise().then(function(a){var e=a[0].map;google.maps.event.addListener(M.data.map.drawingManagerControl.getDrawingManager(),"circlecomplete",function(a){y(a)}),google.maps.event.addListener(M.data.map.drawingManagerControl.getDrawingManager(),"polygoncomplete",function(a){F(a)}),google.maps.event.addListener(M.data.map.drawingManagerControl.getDrawingManager(),"rectanglecomplete",function(a){v(a)}),google.maps.event.addListener(M.data.map.drawingManagerControl.getDrawingManager(),"overlaycomplete",function(a){var e=M.data.map.drawingManagerControl.getDrawingManager();a.type!=google.maps.drawing.OverlayType.MARKER&&(e.setDrawingMode(null),e.setOptions({drawingControl:!1}))});var t=document.createElement("div");new w(t,e);t.index=1,e.controls[google.maps.ControlPosition.TOP_RIGHT].push(t)}),M.doSearch=i,M.addFilter=s,M.removeFilter=d,M.addOrderby=g,M.removeOrderby=c,M.updateQuery=p}];return{restrict:"EA",scope:!0,link:function(a,e,t,r){},templateUrl:"explorer.bootstrap.html",controller:t,controllerAs:"vm",bindToController:{search:"=",loader:"="}}}angular.module("rr.api.v2.explorer.directive.explorer",[]).directive("apiExplorer",a),a.$inject=["ApiConfig","PropertyFactory"]}();
