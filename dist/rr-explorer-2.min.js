...
!function(){"use strict";function a(a,e){var t=["$scope","$interval","$document","PropertyFactory","uiGmapGoogleMapApi","uiGmapIsReady",function(e,t,r,n,l,o){function s(a){if(a){F.data.total_results=-1,F.data.error=null,F.data.results=null,F.data.searching=!0;var e=new Date;n.search(F.data.request).then(function(a){r.scrollToElement(b,70,300);var t=new Date;F.data.error=null,F.data.results=a,F.data.total_results=a.value.length,F.data.searching=!1,u(t.getTime()-e.getTime())},function(a){r.scrollToElement(b,70,300),F.data.query_time=-1,F.data.searching=!1,F.data.error=a?a.error:{}})}}function i(){F.data.searchForm.filter.push({value:"",join:"and"})}function g(a){var e=F.data.searchForm.filter.indexOf(a);F.data.searchForm.filter.splice(e,1),p()}function d(){F.data.searchForm.orderby.push({value:"",direction:"asc"})}function c(a){var e=F.data.searchForm.orderby.indexOf(a);F.data.searchForm.orderby.splice(e,1),p()}function p(){var e="",t=0,r=[0,0,0,0,0];if(""!==F.data.searchForm.select&&(r[0]=1,e+="$select="+F.data.searchForm.select),F.data.searchForm.filter.length)for(r[1]=1,e+=r[0]?"&$filter=":"$filter=",t=0;t<F.data.searchForm.filter.length;t++)e+=F.data.searchForm.filter[t].value,""!==F.data.searchForm.filter[t].value&&t+1<F.data.searchForm.filter.length&&(e+=" "+F.data.searchForm.filter[t].join),t+1<F.data.searchForm.filter.length&&(e+=" ");F.data.searchForm.orderby.length&&(r[2]=1,e+=r[0]+r[1]>0?"&$orderby=":"$orderby=",e+=F.data.searchForm.orderby.map(function(a){return""!==a.value?a.value+" "+a.direction:""}).join(", ")),""!=F.data.searchForm.top&&(r[3]=1,e+=r[0]+r[1]+r[2]>0?"&$top=":"$top=",e+=F.data.searchForm.top),""!=F.data.searchForm.skip&&(r[4]=1,e+=r[0]+r[1]+r[2]+r[3]>0?"&$skip=":"$skip=",e+=F.data.searchForm.skip),F.data.fullRequest=a.apiUrl+"property?"+e,F.data.request=e}function u(a){F.data.query_time=0;var e=5;a>750?F.data.query_time=a:w=t(function(e){F.data.query_time==a?t.cancel(w):F.data.query_time++},e)}function h(a){switch(a){case 1:F.data.map.shape.rectangle=null,F.data.map.shape.polygon=null;break;case 2:F.data.map.shape.circle=null,F.data.map.shape.polygon=null;break;case 3:F.data.map.shape.circle=null,F.data.map.shape.rectangle=null}}function m(a){h(1);var e=null===F.data.map.shape.circle;F.data.map.shape.circle=a;var t=a.getRadius(),r={lat:a.center.lat(),lng:a.center.lng()};console.log(JSON.stringify(r)+" "+t),e&&(F.data.map.shape.circle.addListener("radius_changed",function(){console.log("radius change");var a=F.data.map.shape.circle.getRadius(),e={lat:F.data.map.shape.circle.center.lat(),lng:F.data.map.shape.circle.center.lng()};console.log(JSON.stringify(e)+" "+a)}),F.data.map.shape.circle.addListener("center_changed",function(){console.log("circle center change");var a=F.data.map.shape.circle.getRadius(),e={lat:F.data.map.shape.circle.center.lat(),lng:F.data.map.shape.circle.center.lng()};console.log(JSON.stringify(e)+" "+a)}))}function f(a){h(2);var e=null===F.data.map.shape.rectangle;F.data.map.shape.rectangle=a;var t=[],r=a.getBounds(),n=r.getNorthEast(),l=r.getSouthWest();t.push({lat:n.lat(),lng:l.lng()}),t.push({lat:n.lat(),lng:n.lng()}),t.push({lat:l.lat(),lng:n.lng()}),t.push({lat:l.lat(),lng:l.lng()}),console.log(t),e&&google.maps.event.addListener(F.data.map.shape.rectangle,"bounds_changed",function(){console.log("bounds changed");var a=[],e=F.data.map.shape.rectangle.getBounds(),t=e.getNorthEast(),r=e.getSouthWest();a.push({lat:t.lat(),lng:r.lng()}),a.push({lat:t.lat(),lng:t.lng()}),a.push({lat:r.lat(),lng:t.lng()}),a.push({lat:r.lat(),lng:r.lng()}),console.log(a)})}function y(a){h(3);var e=null===F.data.map.shape.polygon;F.data.map.shape.polygon=a;for(var t=[],r=0;r<a.getPath().getLength();r++){var n=a.getPath().getAt(r);t.push({lat:n.lat(),lng:n.lng()})}console.log(t),e&&(google.maps.event.addListener(F.data.map.shape.polygon.getPath(),"set_at",function(){for(var e=[],t=0;t<a.getPath().getLength();t++){var r=a.getPath().getAt(t);e.push({lat:r.lat(),lng:r.lng()})}}),google.maps.event.addListener(F.data.map.shape.polygon.getPath(),"insert_at",function(){for(var e=[],t=0;t<a.getPath().getLength();t++){var r=a.getPath().getAt(t);e.push({lat:r.lat(),lng:r.lng()})}}),google.maps.event.addListener(F.data.map.shape.polygon.getPath(),"remove_at",function(){for(var e=[],t=0;t<a.getPath().getLength();t++){var r=a.getPath().getAt(t);e.push({lat:r.lat(),lng:r.lng()})}}))}function v(a,e){var t=document.createElement("div");t.style.backgroundColor="#fff",t.style.border="2px solid #fff",t.style.borderRadius="3px",t.style.boxShadow="0 2px 6px rgba(0,0,0,.3)",t.style.cursor="pointer",t.style.marginBottom="22px",t.style.marginRight="10px",t.style.marginTop="10px",t.style.textAlign="center",t.title="Click to delete current selected shape",a.appendChild(t);var r=document.createElement("div");r.style.color="rgb(25,25,25)",r.style.fontFamily="Roboto,Arial,sans-serif",r.style.fontSize="16px",r.style.lineHeight="38px",r.style.paddingLeft="5px",r.style.paddingRight="5px",r.innerHTML="Delete Shape",t.appendChild(r),t.addEventListener("click",function(){F.data.map.shape.circle&&F.data.map.shape.circle.setMap(null),F.data.map.shape.rectangle&&F.data.map.shape.rectangle.setMap(null),F.data.map.shape.polygon&&F.data.map.shape.polygon.setMap(null),F.data.map.drawingManagerControl.getDrawingManager()&&F.data.map.drawingManagerControl.getDrawingManager().setOptions({drawingControl:!0})})}var w,F=this,b=angular.element(document.getElementById("rr-query-results")),M={fillColor:"#ec6952",fillOpacity:.5,editable:!0,draggable:!0,strokeWeight:3,geodesic:!0};e.$watch(angular.bind(this,function(){return this.search}),function(a){a&&"undefined"!==a&&void 0!==a&&"function"!=typeof a&&(F.data.searchForm.select=a.query.select,F.data.searchForm.filter=a.query.filter,F.data.searchForm.orderby=a.query.orderby,F.data.searchForm.top=a.query.top,F.data.searchForm.skip=a.query.skip,F.data.error=null,F.data.results=null,F.data.searching=!1,F.data.total_results=-1,p())}),F.data={searchForm:{select:"",filter:[{value:"",join:"and"}],orderby:[{value:"",direction:"asc"}],top:"",skip:"",geo:{intersects:null,within:null}},map:{center:{latitude:39.9612,longitude:-82.9988},zoom:11,drawingManagerOptions:{},drawingManagerControl:{},bounds:{},shape:{circle:null,rectangle:null,polygon:null}},fullRequest:a.apiUrl+"property?",request:"",results:null,total_results:-1,error:null,searching:!1,query_time:-1},l.then(function(a){F.data.map.drawingManagerOptions={drawingMode:google.maps.drawing.OverlayType.MARKER,drawingControl:!0,drawingControlOptions:{position:google.maps.ControlPosition.TOP_CENTER,drawingModes:[google.maps.drawing.OverlayType.CIRCLE,google.maps.drawing.OverlayType.POLYGON,google.maps.drawing.OverlayType.RECTANGLE]},polygonOptions:M,circleOptions:M,rectangleOptions:M}}),o.promise().then(function(a){var e=a[0].map;google.maps.event.addListener(F.data.map.drawingManagerControl.getDrawingManager(),"circlecomplete",function(a){m(a)}),google.maps.event.addListener(F.data.map.drawingManagerControl.getDrawingManager(),"polygoncomplete",function(a){y(a)}),google.maps.event.addListener(F.data.map.drawingManagerControl.getDrawingManager(),"rectanglecomplete",function(a){f(a)}),google.maps.event.addListener(F.data.map.drawingManagerControl.getDrawingManager(),"overlaycomplete",function(a){var e=F.data.map.drawingManagerControl.getDrawingManager();a.type!=google.maps.drawing.OverlayType.MARKER&&(e.setDrawingMode(null),e.setOptions({drawingControl:!1}))});var t=document.createElement("div");new v(t,e);t.index=1,e.controls[google.maps.ControlPosition.TOP_RIGHT].push(t)}),F.doSearch=s,F.addFilter=i,F.removeFilter=g,F.addOrderby=d,F.removeOrderby=c,F.updateQuery=p}];return{restrict:"EA",scope:!0,link:function(a,e,t,r){},templateUrl:"explorer.bootstrap.html",controller:t,controllerAs:"vm",bindToController:{search:"=",loader:"="}}}angular.module("rr.api.v2.explorer.directive.explorer",[]).directive("apiExplorer",a),a.$inject=["ApiConfig","PropertyFactory"]}();
