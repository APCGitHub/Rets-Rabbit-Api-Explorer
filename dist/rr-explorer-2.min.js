...
!function(){"use strict";function e(e,a){var r=["$scope","$interval","$document","PropertyFactory","uiGmapGoogleMapApi","uiGmapIsReady",function(a,r,t,n,o,l){function i(e){if(e){f.data.total_results=-1,f.data.error=null,f.data.results=null,f.data.searching=!0;var a=new Date;n.search(f.data.request).then(function(e){t.scrollToElement(y,70,300);var r=new Date;f.data.error=null,f.data.results=e,f.data.total_results=e.value.length,f.data.searching=!1,p(r.getTime()-a.getTime())},function(e){t.scrollToElement(y,70,300),f.data.query_time=-1,f.data.searching=!1,f.data.error=e?e.error:{}})}}function d(){f.data.searchForm.filter.push({value:"",join:"and"})}function s(e){var a=f.data.searchForm.filter.indexOf(e);f.data.searchForm.filter.splice(a,1),u()}function g(){f.data.searchForm.orderby.push({value:"",direction:"asc"})}function c(e){var a=f.data.searchForm.orderby.indexOf(e);f.data.searchForm.orderby.splice(a,1),u()}function u(){var a="",r=0,t=[0,0,0,0,0];if(""!==f.data.searchForm.select&&(t[0]=1,a+="$select="+f.data.searchForm.select),f.data.searchForm.filter.length)for(t[1]=1,a+=t[0]?"&$filter=":"$filter=",r=0;r<f.data.searchForm.filter.length;r++)a+=f.data.searchForm.filter[r].value,""!==f.data.searchForm.filter[r].value&&r+1<f.data.searchForm.filter.length&&(a+=" "+f.data.searchForm.filter[r].join),r+1<f.data.searchForm.filter.length&&(a+=" ");f.data.searchForm.orderby.length&&(t[2]=1,a+=t[0]+t[1]>0?"&$orderby=":"$orderby=",a+=f.data.searchForm.orderby.map(function(e){return""!==e.value?e.value+" "+e.direction:""}).join(", ")),""!=f.data.searchForm.top&&(t[3]=1,a+=t[0]+t[1]+t[2]>0?"&$top=":"$top=",a+=f.data.searchForm.top),""!=f.data.searchForm.skip&&(t[4]=1,a+=t[0]+t[1]+t[2]+t[3]>0?"&$skip=":"$skip=",a+=f.data.searchForm.skip),f.data.fullRequest=e.apiUrl+"property?"+a,f.data.request=a}function p(e){f.data.query_time=0;var a=5;e>750?f.data.query_time=e:h=r(function(a){f.data.query_time==e?r.cancel(h):f.data.query_time++},a)}function m(e){var a=null===e;f.data.map.shape.circle=e;var r=e.getRadius(),t={lat:e.center.lat(),lng:e.center.lng()};console.log(JSON.stringify(t)+" "+r),a&&(google.maps.event.addListener(f.data.map.shape.circle,"radius_changed",function(){console.log("radius change");var a=e.getRadius(),r={lat:e.center.lat(),lng:e.center.lng()};console.log(JSON.stringify(r)+" "+a)}),google.maps.event.addListener(f.data.map.shape.circle,"dragend",function(){console.log("dragend change");var a=e.getRadius(),r={lat:e.center.lat(),lng:e.center.lng()};console.log(JSON.stringify(r)+" "+a)}))}var h,f=this,y=angular.element(document.getElementById("rr-query-results")),v={fillColor:"#ec6952",fillOpacity:.5,editable:!0,draggable:!0,zIndex:1,clickable:!1,strokeWeight:3,geodesic:!0};a.$watch(angular.bind(this,function(){return this.search}),function(e){e&&"undefined"!==e&&void 0!==e&&"function"!=typeof e&&(f.data.searchForm.select=e.query.select,f.data.searchForm.filter=e.query.filter,f.data.searchForm.orderby=e.query.orderby,f.data.searchForm.top=e.query.top,f.data.searchForm.skip=e.query.skip,f.data.error=null,f.data.results=null,f.data.searching=!1,f.data.total_results=-1,u())}),f.data={searchForm:{select:"",filter:[{value:"",join:"and"}],orderby:[{value:"",direction:"asc"}],top:"",skip:""},map:{center:{latitude:39.9612,longitude:-82.9988},zoom:11,drawingManagerOptions:{},drawingManagerControl:{},bounds:{},shape:{circle:null,rectangle:null,polygon:null}},fullRequest:e.apiUrl+"property?",request:"",results:null,total_results:-1,error:null,searching:!1,query_time:-1},o.then(function(e){f.data.map.drawingManagerOptions={drawingMode:google.maps.drawing.OverlayType.MARKER,drawingControl:!0,drawingControlOptions:{position:google.maps.ControlPosition.TOP_CENTER,drawingModes:[google.maps.drawing.OverlayType.CIRCLE,google.maps.drawing.OverlayType.POLYGON,google.maps.drawing.OverlayType.RECTANGLE]},polygonOptions:{},circleOptions:{},rectangleOptions:{}},angular.extend(f.data.map.drawingManagerOptions.polygonOptions,v),angular.extend(f.data.map.drawingManagerOptions.circleOptions,v),angular.extend(f.data.map.drawingManagerOptions.rectangleOptions,v)}),l.promise().then(function(e){google.maps.event.addListener(f.data.map.drawingManagerControl.getDrawingManager(),"circlecomplete",function(e){m(e)}),google.maps.event.addListener(f.data.map.drawingManagerControl.getDrawingManager(),"polygoncomplete",function(e){for(var a=[],r=0;r<e.getPath().getLength();r++){var t=e.getPath().getAt(r);a.push({lat:t.lat(),lng:t.lng()})}}),google.maps.event.addListener(f.data.map.drawingManagerControl.getDrawingManager(),"rectanglecomplete",function(e){f.data.map.shape.rectangle=e;var a=[],r=e.getBounds(),t=r.getNorthEast(),n=r.getSouthWest();a.push({lat:t.lat(),lng:n.lng()}),a.push({lat:t.lat(),lng:t.lng()}),a.push({lat:n.lat(),lng:t.lng()}),a.push({lat:n.lat(),lng:n.lng()})}),google.maps.event.addListener(f.data.map.drawingManagerControl.getDrawingManager(),"dragend",function(e){console.log("dragged"),console.log(e)})}),f.doSearch=i,f.addFilter=d,f.removeFilter=s,f.addOrderby=g,f.removeOrderby=c,f.updateQuery=u}];return{restrict:"EA",scope:!0,link:function(e,a,r,t){},templateUrl:"explorer.bootstrap.html",controller:r,controllerAs:"vm",bindToController:{search:"=",loader:"="}}}angular.module("rr.api.v2.explorer.directive.explorer",[]).directive("apiExplorer",e),e.$inject=["ApiConfig","PropertyFactory"]}();
