...
!function(){"use strict";function r(r,e){function a(){function a(){i.data.total_results=-1,e.search(i.data.request).then(function(r){i.data.error=null,i.data.results=r,i.data.total_results=r.value.length},function(r){i.data.error=r.error})}function t(){i.data.searchForm.filter.push({value:"",operator:"",join:"and"})}function o(r){var e=i.data.searchForm.filter.indexOf(r);i.data.searchForm.filter.splice(e,1)}function l(){i.data.searchForm.orderby.push({value:"",direction:"asc"})}function s(r){var e=i.data.searchForm.orderby.indexOf(r);i.data.searchForm.orderby.splice(e,1)}function c(){var e="",a=0,t=[];""!==i.data.searchForm.select&&(e+="$select="+i.data.searchForm.select),i.data.searchForm.filter.length&&(e+=""!==i.data.searchForm.select?"&$filter=":"$filter="),t=i.data.searchForm.filter.map(function(r){return r.value});for(var a=0;a<i.data.searchForm.filter.length;a++)e+=t[a],""!==i.data.searchForm.filter[a].value&&a+1<i.data.searchForm.filter.length&&(e+=" "+i.data.searchForm.filter[a].join),a+1<i.data.searchForm.filter.length&&(e+=" ");i.data.searchForm.orderby.length&&(e+=""!==i.data.searchForm.select&&i.data.searchForm.filter.length?"&$orderby=":"$orderby="),e+=i.data.searchForm.orderby.map(function(r){return""!==r.value?r.value+" "+r.direction:""}).join(", "),""!=i.data.searchForm.top&&(e+="&$top="+i.data.searchForm.top),""!=i.data.searchForm.skip&&(e+="&$skip="+i.data.searchForm.skip),i.data.fullRequest=r.apiUrl+"property?"+e,i.data.request=e}var i=this;i.data={searchForm:{select:"",filter:[{value:"",operator:"",join:"and"}],orderby:[{value:"",direction:"asc"}],top:"",skip:""},fullRequest:r.apiUrl+"property?",request:"",results:null,total_results:-1,error:null},i.search=a,i.addFilter=t,i.removeFilter=o,i.addOrderby=l,i.removeOrderby=s,i.updateQuery=c}return{restrict:"EA",scope:{},templateUrl:"explorer.bootstrap.html",controller:a,controllerAs:"vm",bindToController:!0}}angular.module("rr.api.v2.explorer.directive.explorer",[]).directive("apiExplorer",r),r.$inject=["ApiConfig","PropertyFactory"]}();
