...
!function(){"use strict";function a(a,e){var t=["$scope","$interval","$document","PropertyFactory","uiGmapGoogleMapApi","uiGmapIsReady",function(e,t,r,n,o,l){function i(a){if(a){f.data.total_results=-1,f.data.error=null,f.data.results=null,f.data.searching=!0;var e=new Date;n.search(f.data.request).then(function(a){r.scrollToElement(y,70,300);var t=new Date;f.data.error=null,f.data.results=a,f.data.total_results=a.value.length,f.data.searching=!1,p(t.getTime()-e.getTime())},function(a){r.scrollToElement(y,70,300),f.data.query_time=-1,f.data.searching=!1,f.data.error=a?a.error:{}})}}function s(){f.data.searchForm.filter.push({value:"",join:"and"})}function d(a){var e=f.data.searchForm.filter.indexOf(a);f.data.searchForm.filter.splice(e,1),u()}function g(){f.data.searchForm.orderby.push({value:"",direction:"asc"})}function c(a){var e=f.data.searchForm.orderby.indexOf(a);f.data.searchForm.orderby.splice(e,1),u()}function u(){var e="",t=0,r=[0,0,0,0,0];if(""!==f.data.searchForm.select&&(r[0]=1,e+="$select="+f.data.searchForm.select),f.data.searchForm.filter.length)for(r[1]=1,e+=r[0]?"&$filter=":"$filter=",t=0;t<f.data.searchForm.filter.length;t++)e+=f.data.searchForm.filter[t].value,""!==f.data.searchForm.filter[t].value&&t+1<f.data.searchForm.filter.length&&(e+=" "+f.data.searchForm.filter[t].join),t+1<f.data.searchForm.filter.length&&(e+=" ");f.data.searchForm.orderby.length&&(r[2]=1,e+=r[0]+r[1]>0?"&$orderby=":"$orderby=",e+=f.data.searchForm.orderby.map(function(a){return""!==a.value?a.value+" "+a.direction:""}).join(", ")),""!=f.data.searchForm.top&&(r[3]=1,e+=r[0]+r[1]+r[2]>0?"&$top=":"$top=",e+=f.data.searchForm.top),""!=f.data.searchForm.skip&&(r[4]=1,e+=r[0]+r[1]+r[2]+r[3]>0?"&$skip=":"$skip=",e+=f.data.searchForm.skip),f.data.fullRequest=a.apiUrl+"property?"+e,f.data.request=e}function p(a){f.data.query_time=0;var e=5;a>750?f.data.query_time=a:h=t(function(e){f.data.query_time==a?t.cancel(h):f.data.query_time++},e)}function m(a){f.data.map.shape.circle=a;var e=a.getRadius(),t={lat:a.center.lat(),lng:a.center.lng()};console.log(JSON.stringify(t)+" "+e),google.maps.event.addListener(f.data.map.shape.circle,"radius_changed",function(a){console.log("radius change");var e=a.getRadius(),t={lat:a.center.lat(),lng:a.center.lng()};console.log(JSON.stringify(t)+" "+e)})}var h,f=this,y=angular.element(document.getElementById("rr-query-results")),v={fillColor:"#ec6952",fillOpacity:.5,editable:!0,draggable:!0,zIndex:1,clickable:!1,strokeWeight:3,geodesic:!0};e.$watch(angular.bind(this,function(){return this.search}),function(a){a&&"undefined"!==a&&void 0!==a&&"function"!=typeof a&&(f.data.searchForm.select=a.query.select,f.data.searchForm.filter=a.query.filter,f.data.searchForm.orderby=a.query.orderby,f.data.searchForm.top=a.query.top,f.data.searchForm.skip=a.query.skip,f.data.error=null,f.data.results=null,f.data.searching=!1,f.data.total_results=-1,u())}),f.data={searchForm:{select:"",filter:[{value:"",join:"and"}],orderby:[{value:"",direction:"asc"}],top:"",skip:""},map:{center:{latitude:39.9612,longitude:-82.9988},zoom:11,drawingManagerOptions:{},drawingManagerControl:{},bounds:{},shape:{circle:null,rectangle:null,polygon:null}},fullRequest:a.apiUrl+"property?",request:"",results:null,total_results:-1,error:null,searching:!1,query_time:-1},o.then(function(a){f.data.map.drawingManagerOptions={drawingMode:google.maps.drawing.OverlayType.MARKER,drawingControl:!0,drawingControlOptions:{position:google.maps.ControlPosition.TOP_CENTER,drawingModes:[google.maps.drawing.OverlayType.CIRCLE,google.maps.drawing.OverlayType.POLYGON,google.maps.drawing.OverlayType.RECTANGLE]},polygonOptions:{},circleOptions:{},rectangleOptions:{}},angular.extend(f.data.map.drawingManagerOptions.polygonOptions,v),angular.extend(f.data.map.drawingManagerOptions.circleOptions,v),angular.extend(f.data.map.drawingManagerOptions.rectangleOptions,v)}),l.promise().then(function(a){google.maps.event.addListener(f.data.map.drawingManagerControl.getDrawingManager(),"circlecomplete",function(a){m(a)}),google.maps.event.addListener(f.data.map.drawingManagerControl.getDrawingManager(),"polygoncomplete",function(a){for(var e=[],t=0;t<a.getPath().getLength();t++){var r=a.getPath().getAt(t);e.push({lat:r.lat(),lng:r.lng()})}}),google.maps.event.addListener(f.data.map.drawingManagerControl.getDrawingManager(),"rectanglecomplete",function(a){f.data.map.shape.rectangle=a;var e=[],t=a.getBounds(),r=t.getNorthEast(),n=t.getSouthWest();e.push({lat:r.lat(),lng:n.lng()}),e.push({lat:r.lat(),lng:r.lng()}),e.push({lat:n.lat(),lng:r.lng()}),e.push({lat:n.lat(),lng:n.lng()})}),google.maps.event.addListener(f.data.map.shape.rectangle,"bounds_changed",function(a){console.log("bounds changed");var e=[],t=a.getBounds(),r=t.getNorthEast(),n=t.getSouthWest();e.push({lat:r.lat(),lng:n.lng()}),e.push({lat:r.lat(),lng:r.lng()}),e.push({lat:n.lat(),lng:r.lng()}),e.push({lat:n.lat(),lng:n.lng()})}),google.maps.event.addListener(f.data.map.drawingManagerControl.getDrawingManager(),"dragend",function(a){console.log("dragged"),console.log(a)})}),f.doSearch=i,f.addFilter=s,f.removeFilter=d,f.addOrderby=g,f.removeOrderby=c,f.updateQuery=u}];return{restrict:"EA",scope:!0,link:function(a,e,t,r){},templateUrl:"explorer.bootstrap.html",controller:t,controllerAs:"vm",bindToController:{search:"=",loader:"="}}}angular.module("rr.api.v2.explorer.directive.explorer",[]).directive("apiExplorer",a),a.$inject=["ApiConfig","PropertyFactory"]}();
