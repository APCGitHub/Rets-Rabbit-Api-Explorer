...
!function(){"use strict";function r(r,e){var a=["$scope","$interval","$document","PropertyFactory","uiGmapGoogleMapApi",function(e,a,t,o,l){function n(r){if(r){f.data.total_results=-1,f.data.error=null,f.data.results=null,f.data.searching=!0;var e=new Date;o.search(f.data.request).then(function(r){t.scrollToElement(p,70,300);var a=new Date;f.data.error=null,f.data.results=r,f.data.total_results=r.value.length,f.data.searching=!1,h(a.getTime()-e.getTime())},function(r){t.scrollToElement(p,70,300),f.data.query_time=-1,f.data.searching=!1,f.data.error=r?r.error:{}})}}function i(){f.data.searchForm.filter.push({value:"",join:"and"})}function s(r){var e=f.data.searchForm.filter.indexOf(r);f.data.searchForm.filter.splice(e,1),u()}function c(){f.data.searchForm.orderby.push({value:"",direction:"asc"})}function d(r){var e=f.data.searchForm.orderby.indexOf(r);f.data.searchForm.orderby.splice(e,1),u()}function u(){var e="",a=0,t=[0,0,0,0,0];if(""!==f.data.searchForm.select&&(t[0]=1,e+="$select="+f.data.searchForm.select),f.data.searchForm.filter.length)for(t[1]=1,e+=t[0]?"&$filter=":"$filter=",a=0;a<f.data.searchForm.filter.length;a++)e+=f.data.searchForm.filter[a].value,""!==f.data.searchForm.filter[a].value&&a+1<f.data.searchForm.filter.length&&(e+=" "+f.data.searchForm.filter[a].join),a+1<f.data.searchForm.filter.length&&(e+=" ");f.data.searchForm.orderby.length&&(t[2]=1,e+=t[0]+t[1]>0?"&$orderby=":"$orderby=",e+=f.data.searchForm.orderby.map(function(r){return""!==r.value?r.value+" "+r.direction:""}).join(", ")),""!=f.data.searchForm.top&&(t[3]=1,e+=t[0]+t[1]+t[2]>0?"&$top=":"$top=",e+=f.data.searchForm.top),""!=f.data.searchForm.skip&&(t[4]=1,e+=t[0]+t[1]+t[2]+t[3]>0?"&$skip=":"$skip=",e+=f.data.searchForm.skip),f.data.fullRequest=r.apiUrl+"property?"+e,f.data.request=e}function h(r){f.data.query_time=0;var e=5;r>750?f.data.query_time=r:m=a(function(e){f.data.query_time==r?a.cancel(m):f.data.query_time++},e)}var m,f=this,p=angular.element(document.getElementById("rr-query-results"));l.then(function(r){console.log("hallo")}),e.$watch(angular.bind(this,function(){return this.search}),function(r){r&&"undefined"!==r&&void 0!==r&&"function"!=typeof r&&(f.data.searchForm.select=r.query.select,f.data.searchForm.filter=r.query.filter,f.data.searchForm.orderby=r.query.orderby,f.data.searchForm.top=r.query.top,f.data.searchForm.skip=r.query.skip,f.data.error=null,f.data.results=null,f.data.searching=!1,f.data.total_results=-1,u())}),f.data={searchForm:{select:"",filter:[{value:"",join:"and"}],orderby:[{value:"",direction:"asc"}],top:"",skip:""},map:{center:{lat:39.9612,lng:-82.9988},zoom:10},fullRequest:r.apiUrl+"property?",request:"",results:null,total_results:-1,error:null,searching:!1,query_time:-1},f.doSearch=n,f.addFilter=i,f.removeFilter=s,f.addOrderby=c,f.removeOrderby=d,f.updateQuery=u,leafletData.getMap("rr-map").then(function(r){})}];return{restrict:"EA",scope:!0,link:function(r,e,a,t){},templateUrl:"explorer.bootstrap.html",controller:a,controllerAs:"vm",bindToController:{search:"=",loader:"="}}}angular.module("rr.api.v2.explorer.directive.explorer",[]).directive("apiExplorer",r),r.$inject=["ApiConfig","PropertyFactory"]}();
