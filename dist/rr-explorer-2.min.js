...
!function(){"use strict";function r(r){function e(){function e(){}function a(){i.data.searchForm.filter.push({value:"",operator:"",join:"and"})}function t(r){var e=i.data.searchForm.filter.indexOf(r);i.data.searchForm.filter.splice(e,1)}function o(){i.data.searchForm.orderby.push({value:""})}function l(){console.log("building the query");var r="",e=0,a=[];""!==i.data.searchForm.select&&(r+="$select="+i.data.searchForm.select),i.data.searchForm.filter.length&&(r+=""!==i.data.searchForm.select?"&$filter=":"$filter="),a=i.data.searchForm.filter.map(function(r){return r.value});for(var e=0;e<i.data.searchForm.filter.length;e++)r+=a[e],""!==i.data.searchForm.filter[e].value&&e+1<i.data.searchForm.filter.length&&(r+=" "+i.data.searchForm.filter[e].join),e+1<i.data.searchForm.filter.length&&(r+=" ");i.data.searchForm.orderby.length&&(r+=""!==i.data.searchForm.select&&i.data.searchForm.filter.length?"&$ordery=":"$orderby="),r+=i.data.searchForm.orderby.map(function(r){return""!==r.value?r.value+" "+r.direction:""}).join(", "),""!=i.data.searchForm.top&&(r+="$top="+i.data.searchForm.top),""!=i.data.searchForm.skip&&(r+="$skip="+i.data.searchForm.skip),i.data.request=r}var i=this;i.data={searchForm:{select:"",filter:[{value:"",operator:"",join:"and"}],orderby:[{value:"",direction:"asc"}],top:"",skip:""},request:r.apiUrl+"property?",results:[]},i.search=e,i.addFilter=a,i.removeFilter=t,i.addOrderby=o,i.updateQuery=l}return{restrict:"EA",scope:{},templateUrl:"explorer.bootstrap.html",controller:e,controllerAs:"vm",bindToController:!0}}angular.module("rr.api.v2.explorer.directive.explorer",[]).directive("apiExplorer",r),r.$inject=["ApiConfig"]}();
